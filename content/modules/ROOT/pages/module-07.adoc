...
To begin, lets start our OVN environment by moving into the ModuleOVN directory and executing the `/home/lab-user/ModuleOVN/ovs-dbg/bin/ovs-offline -w /home/lab-user/ModuleOVN/ovs-offline start` script.

[source,console]
----
[lab-user@rhel9 ~]$ cd ModuleOVN
----

[source,console]
----
[lab-user@rhel9 ModuleOVN]$ /home/lab-user/ModuleOVN/ovs-dbg/bin/ovs-offline -w /home/lab-user/ModuleOVN/ovs-offline start
...
When that is finished, you should see 4 pods running with `podman ps`.

[source,console]
----
[lab-user@rhel9 ModuleOVN]$ podman ps
...
The last step is to move into the pre-created execution environment by executing `source /tmp/ovs-offline/bin/activate`. This produces a number of `alias` commands to allow for easier execution of commands against our OVN environment.

[source,console]
----
[lab-user@rhel9 ModuleOVN]$ source /tmp/ovs-offline/bin/activate
...
[source,console]
----
(ovs-offline) [lab-user@rhel9 ModuleOVN]$ ovn-trace --no-leader-only  --db unix:/var/run/ovn/ovnsb_db.sock worker-5.ocpv.tamlab.rdu2.redhat.com 'inport=="frontend_sitea-frontend-8798d9cdc-sq7dk" && eth.src==0a:58:0a:81:06:11 && eth.dst==0a:58:0a:81:06:01 && ip4.src==10.129.6.17 && ip4.dst==10.129.6.15 && ip.ttl==64 && tcp.dst==80 && tcp.src==52888'
----
...
[source,console]
----
(ovs-offline) [lab-user@rhel9 ModuleOVN]$ ovn-nbctl lr-list
...
(ovs-offline) [lab-user@rhel9 ModuleOVN]$ ovn-nbctl ls-list
...
Because a cluster is going to have a lot of ports, we focus in with a `grep` on our specifc pods.

[source,console]
----
(ovs-offline) [lab-user@rhel9 ModuleOVN]$ ovn-nbctl lsp-list worker-5.ocpv.tamlab.rdu2.redhat.com | grep 'siteb\|sitea\|network-policy-demo-monitor\|external'
...
These outports correspond to portgroups and are created for each `NetworkPolicy` in your cluster. All pods selected by the `podSelector` of that `NetworkPolicy` are added to the portgroup’s port list.

Let's take a look at our promlematic `portgroup`.

[source,console]
----
(ovs-offline) [lab-user@rhel9 ModuleOVN]$ ovn-nbctl find Port_Group name="a3640229366109469754"
...
[source,console]
----
(ovs-offline) [lab-user@rhel9 ModuleOVN]$ ovn-nbctl list ACL 0cbe6f52-9cf1-4584-9237-7c23f65c82f1
...
(ovs-offline) [lab-user@rhel9 ModuleOVN]$ ovn-nbctl list ACL 1fdd654d-f251-4e58-b4ce-d57804f1ba9b
...
Now that we know the issue, we can easily fix that with a new NetPol to allow traffic between pods in the namespace.
