...
From the output, you will notice one of the logical switches, *worker-5.ocpv.tamlab.rdu2.redhat.com* is the switch we used in our *ovn-trace*. So lets take a closer look at that switch by listing at the attached *logical switch ports (lsp)*.
...
Logical Switch Port objects, defined by UUID and name, where the name maps to a running pod within the cluster.
...
Now that we have taken a look and validated our LS, LR and LSP objects, we can come back to our trace. You can see traffic going to *outport == @a3640229366109469754* before it is marked as dropped.

Each *outport* corresponds to a *portgroup* and get created for every unqiue *NetworkPolicy* in your cluster. Every pod selected by the *podSelector* of that *NetworkPolicy* is added to the *portgroup* port list.
...
[NOTE]
=====
Because this is a static lab, we can not directly demonstrate the resolution of the issue. So you can see the difference, we have included the output and analysis of a successful trace after applying the above *NetworkPolicy*.
=====
...
The *outport*, like above, corresponds to a *portgroup*. Upon inspection, you can see that this *portgroup* comes from our new *NetworkPolicy* -> *NetworkPolicy:frontend:allow-same-namespace*.
...
On that portgroup, a new *ACL* exists. This *ACL* now has *allow-related* for any *to-lport* traffic that matches the *ip4.src* and outport for our pods in the *frontend* *Namespace*.
