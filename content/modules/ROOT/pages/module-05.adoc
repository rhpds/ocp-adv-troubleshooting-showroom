...
I removed the first old master node and the API became unaccessable. I believe our cluster is having issues running with just two masters.
************************************************
...
[NOTE]
====
The parsing of `subjectaccessreviews` is very slow and does not yield anything useful. Feel free to skip it.
...
With this information, we were able to have an informated discussion with the customer and they were able to take to data to the 3rd party vendor and get the issue addressed to avoid such an API storm in the future.

*How did we recover the cluster?*

With no way of shutting down workloads or disabling the `Operator`, we had to get creative to stabalize the `Control Plane`.

We applied 2 `iptables` rules on all of the `worker` nodes in the cluster.

This blocked both the `kubelet` and running `Pods` from sending traffic to the API and subsequently allow for the API to stabalize and recover.

Once the API was healthy, we removed the `iptables` rules in blocks of 5, allowing nodes to come back and workloads to fully recover.
