...
I have removed the first old master node. Then the API became unaccessable. I believe our cluster is having issues running with just two masters.
************************************************
...
[source,bash]
----
tail -n 1 cluster-master-0[1-3].dmz-audit.log
----
...
Similarily, you can run the same command, using `--by=user`.
...
Again, none of those stick out as glaring issues, but lets dig into the top resources. For this we can use the `--resource=` flag, in addition to `--by=` and `-o top` to down on a specific resource.
...
We can see the `subjectaccessreviews` activity is mostly coming from the `openshift-apiserver`, which you would expect.

[source,bash]
----
252760x        system:serviceaccount:openshift-apiserver:openshift-apiserver-sa
105137x        system:serviceaccount:openshift-monitoring:metrics-server
12701x         system:serviceaccount:openshift-oauth-apiserver:oauth-apiserver-sa
2191x          system:serviceaccount:openshift-logging:cluster-logging-operator
1900x          system:serviceaccount:openshift-monitoring:thanos-querier
1444x          system:serviceaccount:openshift-keda:keda-operator
1350x          system:serviceaccount:openshift-machine-config-operator:machine-config-daemon
1292x          system:serviceaccount:openshift-operator-lifecycle-manager:olm-operator-serviceaccount
1027x          system:serviceaccount:openshift-monitoring:alertmanager-main
629x           system:serviceaccount:openshift-logging:logging-loki-gateway
----
...
Evaluating the output, the most common theme across all of the data points has been activity from `crunchy` / `postgress`. If you refer back to the original `--by=user` output, there was no glaring single user, but 5 of the tp 10 users driving API load were `crunchy` / `postgress`. If we add them all up, a conservative estimate of 621,383 of 4,714,125 request is 13.1% of API traffic. 

So let's take a look at one of the users and see what they are doing. You can do this by passing in the `--user=` flag along with `--by=verb` and `-o top`.
...
. The majority of activity is from `PATCH` and `LIST` commands
. The user is also generating a lot of PV and PVC activity
