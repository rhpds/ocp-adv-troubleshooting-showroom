...
One difference from previous traces you will notice is that we do not set an *eth.dst* MAC address which we will discuss later in the trace analysis.
...
Looking back at our setup, we know that our cluster adds a *deny-by-default* *MultiNetworkPolicy* to every new *Namespace* and that aligns to the policy above that is impacting our external communication.
...
We can also look at the *inport* *a6086215553377573259* as a *portgroup*:
